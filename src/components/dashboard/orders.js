...
export default function Orders() {
  ...
  const rows = [
    ...
    createData(2, 'Trentino con I Balcani', 'Trentino, IT', "Regional cooperation", donationSuccessTrentino),
    createData(
      3,
      'UNESCO',
      'Paris, France',
  ...
  const donate = async (prop) => {
    ...
    if (window.ethereum) {
      ...
      if(result.length > 0) {
        ...
        const contractDonation = new ethers.Contract("0x1fA99bB28cc817aAc2B5D35bE56c0121DAdC8E0F", donationAbi["abi"], currentSigner);
        ...
        <TableBody>
          {rows.map((row) => (
            <TableRow key={row.id}>
              ...
              <TableCell align="right"><Button color="inherit" onClick={ () => donate(row) }>Donate </Button></TableCell>
